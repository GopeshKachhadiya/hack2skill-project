...
async def generate_location_forecast(location: str, horizon_hours: int = 72) -> Dict:
    ...
    traffic = await fetch_traffic_data(location, "Rotterdam")
    ...
    await _persist_forecasts(location, likelihood_series)
...
def _severity_to_float(severity: str) -> float:
    return {"low": 0.1, "medium": 0.35, "high": 0.65, "critical": 0.9}.get(severity.lower(), 0.1)


async def _persist_forecasts(location: str, series: List[Dict]) -> None:
    if not series:
        return
    try:
        ...
        await ProphetForecast.insert_many(records)
...
async def generate_all_location_forecasts() -> Dict:
    ...
    logger.info(f"All-location forecast done: {len(results['success'])} OK, {len(results['failed'])} failed.")
    return results