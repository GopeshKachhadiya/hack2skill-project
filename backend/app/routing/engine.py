...
class RoutingEngine:
    """Orchestrates the hybrid routing process by syncing with DB state."""
    ...
    def _initialize_static_network(self):
        """Build the base graph from known ports/hubs."""
        ...
        # Create strategic maritime edges using hubs
        # 1. Connect Atlantic Hub to Europe and Africa
        ...
        # 2. Connect Suez to Mediterranean and Indian Ocean
        ...
        # 3. Connect Singapore (Malacca Strait) to Asia and Indian Ocean
        ...
        # 4. Connect regional hubs (European Coast)
    ...
    async def update_dynamic_state(self):
        """Update graph weights based on latest disruptions and weather."""
        try:
            # 1. Fetch latest disruption predictions
            # In a real app, this would be the T-GCN output
            # Here we use our existing Prophet/Weather based predictions
            active_disruptions = await DisruptionPrediction.find({"status": "active"}).to_list()
            
            for d in active_disruptions:
                # Find edges affected by this location
                for edge in self.graph.edges.values():
                    if d.location in [edge.source, edge.target]:
                        # Layer 2: Update edge risk
            ...
            # 2. Update node status from weather
            # (Simplification: Weather affects edges connected to the node)
    ...
    async def get_optimal_route(self, origin: str, destination: str, departure_time: datetime):
        ...
        # Ensure origin/destination exist as nodes (approximate to nearest if not)
        # For now, we assume they are port names
